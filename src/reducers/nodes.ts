...
export const checkBlocks = createAsyncThunk(
  "nodes/checkBlocks",
  async (node: Node) => {
...
export const nodesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    //Blocks
    builder.addCase(checkBlocks.fulfilled, (state, action) => {
      const node = state.list.find((n) => n.url === action.meta.arg.url);
      if (node) {
        node.blockLoading = false;
        node.blocks = action.payload.data;
      }
    });

    builder.addCase(checkBlocks.pending, (state, action) => {
      const node = state.list.find((n) => n.url === action.meta.arg.url);
      if (node) {
        node.blockLoading = true;
      }
    });

    builder.addCase(checkBlocks.rejected, (state, action) => {
      const node = state.list.find((n) => n.url === action.meta.arg.url);
      if (node) {
        node.blockLoading = false;
        node.blocks = [];
